...
public class DowntimeHistoricalService : IDowntimeHistoricalService
{
    private readonly AppDbContext _context;

    public DowntimeHistoricalService(AppDbContext context)
    {
        _context = context;
    }

    public async Task<DowntimeDashboardDto> GetDashboardAnalyticsAsync(DashboardFilterDto filter, CancellationToken cancellationToken)
    {
        ...
        {
            var bucketKey = currentBucket.ToString(formatString);
            buckets[bucketKey] = 0;
            openBuckets[bucketKey] = 0;
            responseBuckets[bucketKey] = new List<double>();
            downtimeBuckets[bucketKey] = 0.0;
        ...
        var rawData = await query
            .Select(t => new { t.CreatedAt, t.Status, t.StartedAt, t.FinishedAt, t.IsLineStopped })
            .ToListAsync(cancellationToken);
        ...
        var timeSeries = buckets.Select(b => new TimeSeriesAnalyticsDto(b.Key, b.Value)).OrderBy(x => x.Date).ToList();
        ...
        return new DowntimeDashboardDto(
            ...
            openSeries, responseSeries, downtimeSeries
        );
    }
}