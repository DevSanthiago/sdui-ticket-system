...
namespace TicketSystem.API.Services
{
    public class ExternalAuthService
    {
        ...
        public async Task<List<ExternalRoleDto>> GetAllRolesAsync(string token)
        {
            var client = CreateAuthenticatedClient(token);

            var response = await client.GetAsync("/access-control/api/v1/roles/all");

            if (!response.IsSuccessStatusCode)
                throw new Exception("Houve um erro interno ao buscar a lista de roles do Access Control.");
