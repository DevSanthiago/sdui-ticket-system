...
|---|---|---|---|---|
| `TicketHub` | `/hubs/tickets` | `TicketCreated(ticketId, departmentId)` | grupo `plant-{plantId}` (admin pode trocar via query; demais usam a claim) | board e kiosks atualizam em tempo real + alerta sonoro |
...
- **Sem state manager global** (decisão registrada — ADR-003): os 3 valores globais vivem no `localStorage` e os interceptors resolvem a injeção. Chaves: `ticket_system_auth_token`, `ticket_system_active_plant`, `ticket_user`, `ticket_system_kiosk`, `ticket_system_sound_alert_*`.
...
- UI esconde ações de gestão; o backend reforça read-only por middleware (ver [SECURITY.md](SECURITY.md#modo-kiosk)).
