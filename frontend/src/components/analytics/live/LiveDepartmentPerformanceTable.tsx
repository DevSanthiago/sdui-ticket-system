import {
    ...
    Heading, Skeleton, HStack, Icon, Text, Badge
} from '@chakra-ui/react';
import { Trophy, Medal } from 'lucide-react';
import { useMinimalTheme } from '../../../hooks/theme/useMinimalTheme';
import { useColorModeValue } from '../../../hooks/theme/useColorModeValue';
import type { DepartmentAnalyticsDto } from '../../../types';

interface LiveDepartmentPerformanceTableProps {
    departmentsData: DepartmentAnalyticsDto[] | undefined;
    loading: boolean;
}

export const LiveDepartmentPerformanceTable = ({ departmentsData, loading }: LiveDepartmentPerformanceTableProps) => {
    const theme = useMinimalTheme();
    const isDarkMode = useColorModeValue(false, true);

    return (
        <Box bg={theme.cardBg} p={6} borderRadius="2xl" borderWidth="1px" borderColor={theme.cardBorder} h="100%">
            ...
            <TableContainer>
                <Table variant="simple" size="sm">
                    ...
                    <Tbody>
                        {loading ? (
                            [1, 2, 3, 4, 5].map(i => (
                                <Tr key={i}><Td><Skeleton height="20px" /></Td><Td><Skeleton height="20px" /></Td><Td><Skeleton height="20px" /></Td></Tr>
                            ))
                        ) : departmentsData?.map((dept, index) => (
                            <Tr key={dept.departmentId} _hover={{ bg: isDarkMode ? "whiteAlpha.50" : "blackAlpha.50" }}>
                                ...
                                <Td isNumeric>
                                    <Badge colorScheme={dept.averageResolutionTimeMinutes > 60 ? "red" : "green"} borderRadius="full" px={2}>
